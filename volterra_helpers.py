...
def checkUser(email, s, c):
    if c['expiry'] < datetime.datetime.now().timestamp():
        createUserCache(s)
    thisUser = next(
        (user for user in c['tenantUsers'] if user['email'].lower() == email.lower()), None)
    if thisUser:
...
def cliAdd(token, tenant, email, first_name, last_name, createNS, oRide):
    ...
    if oRide:  # Handle the override
        if createNS:
            ...
            # TBD: More robust -- check for success
            createdNS = s['log'][-1]['resp']['metadata']['name']
        createUserRoles(email, first_name, last_name, s, createdNS,
                        userExist)  # Create the user with her roles
        return {'status': 'success', 'log': s['log']}
    else:  # Standard use case
        if createNS:
            checkNS(email, s)
            if s['log'][-1]['status'] == 'present':  # Is the NS present?
                # No oRide -- this is fatal
                return {'status': 'failure', 'reason': 'NS already exists', 'log': s['log']}
            else:
                createUserNS(email, s)  # Create the NS
                # TBD: more robust
                createdNS = s['log'][-1]['resp']['metadata']['name']
        if userExist:  # User is present
            # No oRide -- this is fatal
            return {'status': 'failure', 'reason': 'User already exists', 'log': s['log']}
        else:
            createUserRoles(email, first_name, last_name,
                            s, createdNS)  # Create the user
